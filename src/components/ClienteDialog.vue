<template>
  ...
  :header="mode === 'create' ? 'Adicionar Cliente' : 'Editar Cliente'" :style="{ width: '600px' }" rounded modal
   >
    <div class="flex flex-col gap-6">
      <div class="flex flex-col gap-2">
        <label for="nome" class="font-semibold text-sm text-gray-700!">Nome Completo *</label>
        <InputText id="nome" v-model="formData.nome_completo" class="w-full bg-[#f3f3f5]! rounded-xl! px-3 py-2 text-sm"
      ...
      <div class="flex flex-col gap-2">
        ...
        <div v-for="(email, index) in formData.emails" :key="index" class="flex gap-2 items-center mb-2">
          <InputText v-model="formData.emails[index]" placeholder="email@exemplo.com"
            class="w-full bg-[#f3f3f5]! rounded-xl! px-3 py-2 text-sm flex-1"
            :class="{ 'p-invalid': errors[`email_${index}`] }" />
          <Button v-if="formData.emails.length > 1" icon="pi pi-trash" severity="danger" class="text-red-900!" text
      ...
      <div class="flex flex-col gap-2">
        ...
        <div v-for="(telefone, index) in formData.telefones" :key="index" class="flex gap-2 items-center mb-2">
          ...
          <Button v-if="formData.telefones.length > 1" icon="pi pi-trash" severity="danger" text class="text-red-900"
...
watch(() => props.visible, (newVal) => {
  if (newVal) {
    if (props.mode === 'edit' && props.cliente) {
      formData.value = {
        id: props.cliente.id,
        nome_completo: props.cliente.nome_completo,
        emails: [...props.cliente.emails],
